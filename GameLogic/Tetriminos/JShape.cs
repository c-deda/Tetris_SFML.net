namespace Tetris
{
    public class JShape : Tetrimino
    {
        public JShape() : base()
        {
            for (int i = 0; i < 4; i++)
            {
                _blocks[i] = new Block(BlockValue.JShape, new Position(Constants.DespawnedPosition, Constants.DespawnedPosition));
            }
        }

        public override void Spawn()
        {
            _blocks[0].Move(new Position(6, 1));
            _blocks[1].Move(new Position(5, 1));
