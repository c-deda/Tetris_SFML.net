namespace Tetris
{
    public class TShape : Tetrimino
    {
        public TShape() : base()
        {
            for (int i = 0; i < 4; i++)
            {
                blocks[i] = new Block(BlockValue.TSHAPE, new Position(Constants.DESPAWNED_POS, Constants.DESPAWNED_POS));
            }
        ...
        public override void Spawn()
        {
            ...
            blocks[3].Move(new Position(4, 0));
        }
        ...
        public override void Rotate(bool clockwise)
        {
            ...
            switch (rotationIndex)
            {
                case 0:
                    ...
                    blocks[3].Move(new Position(pivot.x, pivot.y - 1));
                    break;
                case 1:
                    blocks[0].Move(new Position(pivot.x, pivot.y - 1));
                    blocks[2].Move(new Position(pivot.x, pivot.y + 1));
                    blocks[3].Move(new Position(pivot.x + 1, pivot.y));
                    break;
                case 2:
                    blocks[0].Move(new Position(pivot.x + 1, pivot.y));
                    blocks[2].Move(new Position(pivot.x - 1, pivot.y));
