namespace Tetris
{
    public class JShape : Tetrimino
    {
        public JShape() : base()
        {
            for (int i = 0; i < 4; i++)
            {
                blocks[i] = new Block(BlockValue.JSHAPE, new Position(Constants.DESPAWNED_POS, Constants.DESPAWNED_POS));
            }
        ...
        {
            blocks[0].Move(new Position(6, 1));
            blocks[1].Move(new Position(5, 1));
